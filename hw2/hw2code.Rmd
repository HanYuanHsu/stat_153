...
## a
Plot the periodogram of the data and comment on its notable features. (3
points).
```{r}
n_half = floor(n/2)
...
In the following code, `post.samples[ ,1:5]` stores samples of $f, \vec\beta$ from the joint posterior distribution $f, \vec\beta\;|\; \vec y$, and `post.samples[ ,c(1,6)]` stores samples of $f, \sigma$ from the joint posterior distribution $f, \sigma\;|\; \vec y$.
...
```{r}
plot(my.grid, as.numeric(lapply(my.grid, FUN=post.f)), type='l', ylab='posterior pdf of f')
```
...
```{r}
N = 2000 #number of posterior samples
# in post.samples, column 1 is for samples from the distribution f | y
# column 2 to 6 is for samples from (beta_0, ..., beta_4) | f, y
# where f is fixed at the value given by column 1.
post.samples = matrix(-1, nrow=N, ncol=6)
