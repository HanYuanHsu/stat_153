...
p = 8
X = matrix(1, nrow = n, ncol = p)
logpost = function(f_1, f_2, f_3) # fs: a vector of f_1, f_2, f_3
{
    X[,2] = tme
...
We need to discretize the frequencies $f_1, f_2, f_3$ to do Bayesian inference. From the periodogram, we see that a good range of the frequencies to consider is from 10/n to 60/n
```{r}
f1.grid = seq(10/n, 60/n, by=.5/n)
f2.grid = seq(10/n, 60/n, by=.5/n)
f3.grid = seq(10/n, 60/n, by=.5/n)
g = expand.grid(x=f1.grid, y=f2.grid, z = f3.grid)

for(i in 1:nrow(g)) g$lp[i] = logpost(g$x[i], g$y[i], g$z[i])
